...
func DefaultApplyFieldMaskHost(ctx context.Context, patchee *Host, patcher *Host, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Host, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedDistance && strings.HasPrefix(f, prefix+"Distance.") {
			...
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"Distance."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.Distance, patchee.Distance, childMask); err != nil {
				return nil, nil
			}
		}
		...
		if !updatedIPIDSequence && strings.HasPrefix(f, prefix+"IPIDSequence.") {
			...
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"IPIDSequence."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.IPIDSequence, patchee.IPIDSequence, childMask); err != nil {
				return nil, nil
			}
		}
		...
		if !updatedTCPSequence && strings.HasPrefix(f, prefix+"TCPSequence.") {
			...
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"TCPSequence."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.TCPSequence, patchee.TCPSequence, childMask); err != nil {
				return nil, nil
			}
		}
		...
		if !updatedTCPTSSequence && strings.HasPrefix(f, prefix+"TCPTSSequence.") {
			...
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"TCPTSSequence."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.TCPTSSequence, patchee.TCPTSSequence, childMask); err != nil {
				return nil, nil
			}
		}
