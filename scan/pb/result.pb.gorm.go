...
type ResultORM struct {
	Address   *pb1.AddressORM `gorm:"foreignKey:AddressId;references:Id"`
	AddressId *string
	Data      string
	Host      *pb.HostORM `gorm:"foreignKey:HostId;references:Id"`
	HostId    *string
	Id        string      `gorm:"type:uuid;primaryKey"`
	Port      *pb.PortORM `gorm:"foreignKey:PortId;references:Id"`
	PortId    *string
	Service   *pb1.ServiceORM `gorm:"foreignKey:ServiceId;references:Id"`
	ServiceId *string
}
...
func DefaultApplyFieldMaskResult(ctx context.Context, patchee *Result, patcher *Result, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Result, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedHost && strings.HasPrefix(f, prefix+"Host.") {
			...
			if patchee.Host == nil {
				patchee.Host = &pb.Host{}
			}
			if o, err := pb.DefaultApplyFieldMaskHost(ctx, patchee.Host, patcher.Host, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Host.", db); err != nil {
				return nil, err
			} else {
				patchee.Host = o
			}
		...
		if !updatedAddress && strings.HasPrefix(f, prefix+"Address.") {
			...
			if patchee.Address == nil {
				patchee.Address = &pb1.Address{}
			}
			if o, err := pb1.DefaultApplyFieldMaskAddress(ctx, patchee.Address, patcher.Address, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Address.", db); err != nil {
				return nil, err
			} else {
				patchee.Address = o
			}
		...
		if !updatedPort && strings.HasPrefix(f, prefix+"Port.") {
			...
			if patchee.Port == nil {
				patchee.Port = &pb.Port{}
			}
			if o, err := pb.DefaultApplyFieldMaskPort(ctx, patchee.Port, patcher.Port, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Port.", db); err != nil {
				return nil, err
			} else {
				patchee.Port = o
			}
		...
		if !updatedService && strings.HasPrefix(f, prefix+"Service.") {
			...
			if patchee.Service == nil {
				patchee.Service = &pb1.Service{}
			}
			if o, err := pb1.DefaultApplyFieldMaskService(ctx, patchee.Service, patcher.Service, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Service.", db); err != nil {
				return nil, err
			} else {
				patchee.Service = o
			}
