...
// This file implements the ingest fold: the non-destructive match/merge that lets
// several scans (or several scanners) contribute to the same host/port/service
// objects without either duplicating them or dropping data. It is the in-memory,
// PB-level counterpart to the DB dedup layer and follows the contract in DEDUP.md:
//
...
//
// Every merge returns whether it changed the destination, so callers can skip
// no-op writes (idempotent re-import issues zero updates).

// AddHosts folds one or more hosts into this Run, deduplicating and merging by
// natural key. It is the bulk entry point behind the import path: the Hosts of a
// freshly parsed scan.Run (e.g. from nmap.FromXML) are folded in one by one, so an
// import that overlaps hosts already in the Run enriches them instead of duplicating.
...
//
// Identity — keyed, PB-level (mirrors the strong keys of host.AreHostsIdentical) --
//

// sameHost reports whether two in-memory hosts denote the same machine, using
// natural keys only (DEDUP.md §2, keyed-first): MAC is definitive, otherwise a
// shared address is decisive. A shared hostname alone is deliberately NOT enough to
// merge — virtual hosts share names — so per the no-false-merge asymmetry we would
// rather split than wrongly collapse two hosts.
...
// mergeHostInto folds src into dst in place, by field class, without losing data.
func mergeHostInto(dst, src *host.Host) (changed bool) {
	if dst == nil || src == nil {
		return false
	}
