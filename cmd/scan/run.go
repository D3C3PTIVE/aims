...
// `aims scan run nmap -sT -p1-1000 scanme.nmap.org`. The command blocks until the scan finishes;
// Ctrl-C detaches (the scan keeps running server-side — reattach with `scan attach`), and a shell
// `&` backgrounds the client. The one aims-owned token is `--json` (ndjson stream).
func runNmapCommand(con *client.Client) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "Run an nmap scan by passing arguments straight through to nmap. The scan runs on the\n" +
			...
			"Blocks until the scan finishes: a live dashboard (progress bar + hosts as they are found)\n" +
			"when stdout is a terminal, a plain line log when piped. Ctrl-C detaches (the scan keeps\n" +
			"running server-side — reattach with `scan attach`); a shell `&` backgrounds the client.\n" +
			"aims-owned flag: --json streams each update as a line of JSON (ndjson). XML output is\n" +
			"added automatically; results are stored.",
...
// It is the same raw-passthrough shape as nmap (DisableFlagParsing, one aims-owned --json), driven by
// the masscan driver which appends XML output automatically and folds the result through the shared
// nmap XML parser. Completion (completeRunMasscan) reuses every value completer — targets, ports,
// interface — behind a masscan-specific dispatch.
func runMasscanCommand(con *client.Client) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "Run a masscan scan by passing arguments straight through to masscan. The scan runs on\n" +
			...
			"Blocks until the scan finishes: a live dashboard when stdout is a terminal, a plain line\n" +
			"log when piped. Ctrl-C detaches (the scan keeps running server-side — reattach with\n" +
			"`scan attach`); a shell `&` backgrounds the client. aims-owned flag: --json (ndjson\n" +
			"stream). XML output is added automatically; results are stored.",
		DisableFlagParsing: true,
		RunE: func(command *cobra.Command, args []string) error {
...
// help case, strips the one aims-owned --json token (DisableFlagParsing leaves no cobra flag to
// bind), forwards the rest verbatim to the server-side driver over the streaming Run RPC, and renders
// the stream. The command blocks to completion; Ctrl-C detaches the still-running server-side job (a
// shell `&` backgrounds the client). scanner is the driver name the server resolves (see scannerFor).
func runScanner(command *cobra.Command, con *client.Client, scanner string, args []string) error {
	...
	// --json is the sole aims-owned token. With DisableFlagParsing on there is no cobra flag to bind,
	// so strip it by hand (long form only — a scanner's short flags may mean something else) and
