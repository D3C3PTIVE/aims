...
type PortORM struct {
	...
	Owner     string
	Protocol  string
	Reasons   []*ReasonORM      `gorm:"foreignKey:PortId;references:Id"`
...
type ExtraPortORM struct {
	...
	HostId  *string
	Id      string       `gorm:"type:uuid;primaryKey"`
	Reasons []*ReasonORM `gorm:"foreignKey:ExtraPortId;references:Id"`
	State   string
}
...
func DefaultStrictUpdatePort(ctx context.Context, in *Port, db *gorm.DB) (*Port, error) {
	...
	filterReasons.PortId = new(string)
	*filterReasons.PortId = ormObj.Id
	if err = db.Where(filterReasons).Delete(ReasonORM{}).Error; err != nil {
		return nil, err
	}
	if err = db.Model(&ormObj).Association("Scripts").Replace(ormObj.Scripts); err != nil {
		return nil, err
	}
	ormObj.Scripts = nil
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedService && strings.HasPrefix(f, prefix+"Service.") {
			...
			if patchee.Service == nil {
				patchee.Service = &pb.Service{}
			}
			if o, err := pb.DefaultApplyFieldMaskService(ctx, patchee.Service, patcher.Service, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Service.", db); err != nil {
				return nil, err
			} else {
				patchee.Service = o
			}
...
func DefaultStrictUpdateExtraPort(ctx context.Context, in *ExtraPort, db *gorm.DB) (*ExtraPort, error) {
	...
	filterReasons.ExtraPortId = new(string)
