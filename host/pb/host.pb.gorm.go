...
type HostORM struct {
	Addresses       []*pb.AddressORM `gorm:"foreignKey:Id;references:Id;many2many:host_addresses;joinForeignKey:HostId;joinReferences:AddressId"`
	Arch            string
	Comm            string
	Comment         string
	CreatedAt       *time.Time
	Distance        *pb.DistanceORM `gorm:"foreignKey:DistanceId;references:Id"`
	DistanceId      *string
	EndTime         int64
	...
	ICMPResponse    *pb.ICMPResponseORM `gorm:"foreignKey:ICMPResponseId;references:Id"`
	ICMPResponseId  *string
	IPIDSequence    *pb.IPIDSequenceORM `gorm:"foreignKey:IPIDSequenceId;references:Id"`
	...
	Sources         []*pb1.SourceORM `gorm:"foreignKey:Id;references:Id;many2many:host_sources;joinForeignKey:HostId;joinReferences:SourceId"`
	StartTime       int64
	Status          *StatusORM         `gorm:"foreignKey:HostId;references:Id"`
	TCPSequence     *pb.TCPSequenceORM `gorm:"foreignKey:TCPSequenceId;references:Id"`
	TCPSequenceId   *string
	TCPTSSequence   *pb.TCPTSSequenceORM `gorm:"foreignKey:TCPTSSequenceId;references:Id"`
	TCPTSSequenceId *string
	Testing         *pb.TimesORM `gorm:"foreignKey:TimesId;references:Id"`
	TimesId         *string
	Trace           *pb.TraceORM `gorm:"foreignKey:TraceId;references:Id"`
	TraceId         *string
	UpdatedAt       *time.Time
...
func (m *Host) ToORM(ctx context.Context) (HostORM, error) {
	...
	for _, v := range m.Processes {
		if v != nil {
			if tempProcesses, cErr := v.ToORM(ctx); cErr == nil {
				to.Processes = append(to.Processes, &tempProcesses)
			} else {
				return to, cErr
			}
		} else {
...
func (m *HostORM) ToPB(ctx context.Context) (Host, error) {
	...
	for _, v := range m.Processes {
		if v != nil {
			if tempProcesses, cErr := v.ToPB(ctx); cErr == nil {
				to.Processes = append(to.Processes, &tempProcesses)
			} else {
				return to, cErr
			}
...
func DefaultStrictUpdateHost(ctx context.Context, in *Host, db *gorm.DB) (*Host, error) {
	...
	*filterPorts.HostId = ormObj.Id
	...
	filterProcesses := ProcessORM{}
	if ormObj.Id == "" {
		return nil, errors.EmptyIdError
	}
	filterProcesses.HostId = new(string)
	*filterProcesses.HostId = ormObj.Id
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedDistance && strings.HasPrefix(f, prefix+"Distance.") {
			...
			if patchee.Distance == nil {
				patchee.Distance = &pb.Distance{}
			}
			if o, err := pb.DefaultApplyFieldMaskDistance(ctx, patchee.Distance, patcher.Distance, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Distance.", db); err != nil {
				return nil, err
			} else {
				patchee.Distance = o
			}
		...
		if !updatedIPIDSequence && strings.HasPrefix(f, prefix+"IPIDSequence.") {
			...
			if patchee.IPIDSequence == nil {
				patchee.IPIDSequence = &pb.IPIDSequence{}
			}
			if o, err := pb.DefaultApplyFieldMaskIPIDSequence(ctx, patchee.IPIDSequence, patcher.IPIDSequence, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"IPIDSequence.", db); err != nil {
				return nil, err
			} else {
		...
		if !updatedTCPSequence && strings.HasPrefix(f, prefix+"TCPSequence.") {
			...
			if patchee.TCPSequence == nil {
				patchee.TCPSequence = &pb.TCPSequence{}
			}
			if o, err := pb.DefaultApplyFieldMaskTCPSequence(ctx, patchee.TCPSequence, patcher.TCPSequence, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"TCPSequence.", db); err != nil {
				return nil, err
			} else {
		...
		if !updatedTCPTSSequence && strings.HasPrefix(f, prefix+"TCPTSSequence.") {
			...
			if patchee.TCPTSSequence == nil {
				patchee.TCPTSSequence = &pb.TCPTSSequence{}
			}
			if o, err := pb.DefaultApplyFieldMaskTCPTSSequence(ctx, patchee.TCPTSSequence, patcher.TCPTSSequence, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"TCPTSSequence.", db); err != nil {
				return nil, err
			} else {
		...
		if !updatedICMPResponse && strings.HasPrefix(f, prefix+"ICMPResponse.") {
			...
			if patchee.ICMPResponse == nil {
				patchee.ICMPResponse = &pb.ICMPResponse{}
			}
			if o, err := pb.DefaultApplyFieldMaskICMPResponse(ctx, patchee.ICMPResponse, patcher.ICMPResponse, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"ICMPResponse.", db); err != nil {
				return nil, err
			} else {
		...
		if !updatedTesting && strings.HasPrefix(f, prefix+"Testing.") {
			...
			if patchee.Testing == nil {
				patchee.Testing = &pb.Times{}
			}
			if o, err := pb.DefaultApplyFieldMaskTimes(ctx, patchee.Testing, patcher.Testing, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Testing.", db); err != nil {
				return nil, err
			} else {
				patchee.Testing = o
			}
		...
		if !updatedTrace && strings.HasPrefix(f, prefix+"Trace.") {
			...
			if patchee.Trace == nil {
				patchee.Trace = &pb.Trace{}
			}
			if o, err := pb.DefaultApplyFieldMaskTrace(ctx, patchee.Trace, patcher.Trace, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Trace.", db); err != nil {
				return nil, err
			} else {
				patchee.Trace = o
			}
