...
type UserORM struct {
	CreatedAt *time.Time
	Groups    []*GroupORM `gorm:"foreignKey:Id;references:Id;many2many:user_groups;joinForeignKey:UserId;joinReferences:GroupId"`
	HostId    *string
...
type GroupORM struct {
	...
	GID       int32
	Id        string     `gorm:"type:uuid;primaryKey"`
	Members   []*UserORM `gorm:"foreignKey:Id;references:Id;many2many:group_users;joinForeignKey:GroupId;joinReferences:UserId"`
	Name      string
	UpdatedAt *time.Time
}
...
func DefaultStrictUpdateUser(ctx context.Context, in *User, db *gorm.DB) (*User, error) {
	...
	if err = db.Model(&ormObj).Association("Groups").Replace(ormObj.Groups); err != nil {
		return nil, err
	}
	ormObj.Groups = nil
...
func DefaultStrictUpdateGroup(ctx context.Context, in *Group, db *gorm.DB) (*Group, error) {
	...
	if err = db.Model(&ormObj).Association("Members").Replace(ormObj.Members); err != nil {
		return nil, err
	}
	ormObj.Members = nil
