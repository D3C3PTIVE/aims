...
func DefaultApplyFieldMaskAgent(ctx context.Context, patchee *Agent, patcher *Agent, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Agent, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedUser && strings.HasPrefix(f, prefix+"User.") {
			...
			if patchee.User == nil {
				patchee.User = &pb.User{}
			}
			if o, err := pb.DefaultApplyFieldMaskUser(ctx, patchee.User, patcher.User, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"User.", db); err != nil {
				return nil, err
			} else {
				patchee.User = o
			}
		...
		if !updatedProcess && strings.HasPrefix(f, prefix+"Process.") {
			...
			if patchee.Process == nil {
				patchee.Process = &pb.Process{}
			}
			if o, err := pb.DefaultApplyFieldMaskProcess(ctx, patchee.Process, patcher.Process, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Process.", db); err != nil {
				return nil, err
			} else {
				patchee.Process = o
			}
